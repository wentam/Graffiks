...
public class Mesh {
    ...
    float[] baseVertices = null;
    float[] baseNormals = null;

    public void setInstances(double[][] instances) {
        // vertices and normals make up our base mesh.
        // copy that data if we haven't already
        if (baseVertices == null) {
            baseVertices = Arrays.copyOf(vertices, vertices.length);
        }

        if (baseNormals == null) {
            baseNormals = Arrays.copyOf(normals, normals.length);
        }

        // reallocate to correct length
        if (vertices.length != baseVertices.length*instances.length) {
            vertices = Arrays.copyOf(vertices,baseVertices.length*instances.length);
            bb = ByteBuffer.allocateDirect(vertices.length * 4);
            bb.order(ByteOrder.nativeOrder());
            vertexBuffer = bb.asFloatBuffer();
        }

        if (normals.length != baseNormals.length*instances.length) {
            normals = Arrays.copyOf(normals,baseNormals.length*instances.length);
            bb = ByteBuffer.allocateDirect(normals.length * 4);
            bb.order(ByteOrder.nativeOrder());
            normalBuffer = bb.asFloatBuffer();
        }
        ...
        for (int i = 0; i < instances.length; i++) {
            for (int i2 = 0; i2 < baseVertices.length; i2++) {
                if (indexOffset == 2 && useZ == false) {
                    vertices[startIndex+i2] = baseVertices[i2];
                } else {
                    vertices[startIndex+i2] = baseVertices[i2]+(float)instances[i][indexOffset];
                }
                indexOffset++;
                if (indexOffset >= 3) {
                    indexOffset = 0;
            ...
            startIndex += baseVertices.length;
        }

        vertexCount = vertices.length;
        //Log.i("graffiks","vertices: "+Arrays.toString(vertices));

        // normals
        startIndex = 0;

        for (int i = 0; i < instances.length; i++) {
            for (int i2 = 0; i2 < baseNormals.length; i2++) {
                normals[startIndex+i2] = baseNormals[i2];
            }

            startIndex += baseNormals.length;
        }
