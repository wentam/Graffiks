...
void set_view_matrix (float matrix[],
    ...
    //
    // we need to define the columns x to be a vector to the right, y to be a vector above, and z to be a vector forwards,
    // relative to the "camera"'s angle. these need to be normalized as we get passed vectors of arbitrary length.
    // we also need to set the column t to the location of the camera.

    // the forward vector is the difference between what we're looking at and where we are
    ...
    // to get the correct mangnitude for up we just use the vector product of right and forward
    float up_vector_x = right_vector_y * forward_vector_z - right_vector_z * forward_vector_y;
    float up_vector_y = right_vector_z * forward_vector_x - right_vector_x * forward_vector_z;
    float up_vector_z = right_vector_x * forward_vector_y - right_vector_y * forward_vector_x;

    // and now we write it all to the matrix
    //
    // note that we need to invert the forward vector, as we're moving the world and the not the camera
